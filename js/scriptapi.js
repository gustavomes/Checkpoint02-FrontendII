...
const tarefasPendentes = document.querySelector('#todo');
const tarefasConcluidas = document.querySelector('#completed');
...
btn.addEventListener('click', (event) => {
  ...
  fetch(url) // comunicação assíncrona - envia um GET para a url
    ...
    .then( reposArray => { // quando o objeto json estiver pronto (ele é um array de repositorios!)
      ...
      reposArray.forEach( (repo) => tarefasPendentes.innerHTML += `<li> ${repo.title} </li>` );

                               
     })
    .catch(error => console.log(`houve um erro! ${error}`)); // quando o json estiver pronto, mostre o array de repos no log
    
     //chamar a função addItemTodo com parametro repo.title

      addItemTodo(repo.title);
...
function addItemTodo(repos) {
  var list = document.querySelector('.todo');

  var item = document.createElement('li');
  item.innerText = `${repos}`;
